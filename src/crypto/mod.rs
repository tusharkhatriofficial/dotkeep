...
pub fn prompt_password(prompt: &str) -> Result<String, DotkeepError> {
    ...
    let password = read_password().map_err(|e| DotkeepError::EncryptionError(e.to_string()))?;
    Ok(password)
}
...
pub fn encrypt(key: &[u8; 32], plaintext: &[u8]) -> Result<Vec<u8>, DotkeepError> {
    let rng = SystemRandom::new();

    // Generate a random 12-byte nonce
    let mut nonce_bytes = [0u8; NONCE_LEN];
    rng.fill(&mut nonce_bytes)
        .map_err(|_| DotkeepError::EncryptionError("Failed to generate nonce".to_string()))?;

    // Create the sealing key
    let unbound_key = UnboundKey::new(&aead::AES_256_GCM, key)
        .map_err(|_| DotkeepError::EncryptionError("Invalid key".to_string()))?;
    ...
    sealing_key
        ...
        .map_err(|_| DotkeepError::EncryptionError("Encryption failed".to_string()))?;
...
pub fn decrypt(key: &[u8; 32], encrypted: &[u8]) -> Result<Vec<u8>, DotkeepError> {
    if encrypted.len() < NONCE_LEN + TAG_LEN {
        return Err(DotkeepError::DecryptionError(
    ...
    let unbound_key = UnboundKey::new(&aead::AES_256_GCM, key)
        .map_err(|_| DotkeepError::DecryptionError("Invalid key".to_string()))?;
...
pub fn encrypt_value(key: &[u8; 32], value: &str) -> Result<String, DotkeepError> {
    let encrypted = encrypt(key, value.as_bytes())?;
    Ok(base64_encode(&encrypted))
}

/// Decrypt a base64-encoded ciphertext back to a string.
pub fn decrypt_value(key: &[u8; 32], encoded: &str) -> Result<String, DotkeepError> {
    let encrypted = base64_decode(encoded)?;
    let decrypted = decrypt(key, &encrypted)?;
    String::from_utf8(decrypted).map_err(|e| DotkeepError::DecryptionError(e.to_string()))
}
...
fn base64_decode(hex: &str) -> Result<Vec<u8>, DotkeepError> {
    (0..hex.len())
        .step_by(2)
        .map(|i| {
            u8::from_str_radix(&hex[i..i + 2], 16)
                .map_err(|_| DotkeepError::DecryptionError("Invalid hex".to_string()))
        })
        .collect()
}
